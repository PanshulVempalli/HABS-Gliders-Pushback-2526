                                   Panshul Vempalli's 2025/2026 Season 

# HABS Gliders 34071B — VEX Pushback 2025/26

![VEX Robotics](https://img.shields.io/badge/VEX-Robotics-red) ![Season](https://img.shields.io/badge/Season-Pushback%202025%2F26-orange) ![Team](https://img.shields.io/badge/Team-34071B-blue)

Official season repository for **HABS Gliders 34071B** competing in the VEX Robotics Pushback 2025/26 season.

---

## 📁 Repository Structure

| Folder | Description |
|--------|-------------|
| `robot-code/` | Main robot code using JAR and EZ templates |
| `autonomous/` | Prematch autonomous routines |
| `python-tools/` | Python skills autonomous and utilities |
| `vision/` | VEX VR simulation and vision code |
| `docs/` | Match notes, strategy, and diagrams |

---
## 📂 Projects

| Project | Description | Link |
|---------|-------------|------|
| HABS Gliders Team Website | Official team website built with React | [View →](https://github.com/PanshulVempalli/habs-gliders-34071b) |
| JAR Template | Advanced PID and motion control | [View →](https://github.com/PanshulVempalli/JAR-template-Example-VEX-Code) |
| EZ Template | PROS-based robot code | [View →](https://github.com/PanshulVempalli/EZ-Template_VEX_code) |
| Prematch Auton | Match autonomous routines | [View →](https://github.com/PanshulVempalli/VEX-V5-Prematch-Auton-Example) |
| Python Skills | Skills run code | [View →](https://github.com/PanshulVempalli/VEX-V5-Python-Skills-Auton-Example) |
| VEX VR | Virtual reality simulation | [View →](https://github.com/PanshulVempalli/VEX_Vr_Code_HABS-Gliders) |

---

## 🤖 About the Team

- **Team Number:** 34071B
- **Team Name:** HABS Gliders
- **Game:** Pushback
- **Season:** 2025/26
- **School(s):** HABS Boys and Girls 

---

## 🛠️ Tech Stack

- **C++** — Main robot code (JAR Template & EZ Template)
- **Python** — Skills autonomous and VEX VR
- **PROS** — Robot operating system for EZ Template
- **VEX V5** — Robot hardware platform

---

## 📂 Projects

### Robot Code
- **JAR Template** — Advanced PID and motion control
- **EZ Template** — PROS-based robot code

### Autonomous
- **Prematch Auton** — Match autonomous routines

### Python Tools
- **Skills Autonomous** — Python skills run code
- **VEX VR** — Virtual reality simulation and testing

---

## 🏆 Season Results 

- Qualified for UK Nats and Worlds
- Had a great time 

---
Go check out another team member's VEX content: Kayan Shah - https://github.com/KayanShah

## ❤️ Thank you

- Thank you to Kayan Shah and the rest of the HABS-Gliders team
- Thank you to all HABS staff
- Thank you for all our alliances and competitors for making this season soo fun
- Thank you to VEX and the REC Foundation for making this season possibe 

*Built by Panshul Vempalli — HABS Gliders 34071B*
